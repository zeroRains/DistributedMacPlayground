...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        String type = args[0];
        String in1 = args.length > 1 && args.length<=3 ? args[1] : null;
        String in2 = args.length > 1 && args.length<=3 ? args[2] : null;
        ...
        if (in1 != null && in2 != null) {
            if (_type != MMMethodType.MapMM) {
                ...
                if (runMethod.getOut() != null) {
                    List<Tuple2<MatrixIndexes, MatrixBlock>> tuple2List = runMethod.getOut().collect();
                }
            } else {
                RunMethod runMethod = new RunMethod(sc, _type, row, col, middle, CommonConfig.CacheTpye.LEFT, CommonConfig.SparkAggType.MULTI_BLOCK, path + in1, path + in2);
                runMethod.set_blockSize(blockSize);
                runMethod.execute();
            }
        } else {
